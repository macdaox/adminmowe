...
function queryTimeoutMs() {
  const n = process.env.MYSQL_QUERY_TIMEOUT_MS ? Number(process.env.MYSQL_QUERY_TIMEOUT_MS) : 5000;
  return Number.isFinite(n) && n > 0 ? n : 5000;
}

function isTransientMySQLError(e) {
  const code = e && e.code ? String(e.code) : '';
  const msg = e && e.message ? String(e.message).toLowerCase() : '';
  return (
    ['PROTOCOL_CONNECTION_LOST', 'ECONNRESET', 'ETIMEDOUT', 'EPIPE', 'PROTOCOL_ENQUEUE_AFTER_FATAL_ERROR'].includes(code) ||
...
async function query(pool, sql, params, code) {
  let lastErr = null;
  for (let i = 0; i < 2; i += 1) {
    try {
      return await withTimeout(pool.query(sql, params), queryTimeoutMs(), code || 'mysql_query_timeout');
    } catch (e) {
      lastErr = e;
      if (i > 0 || !isTransientMySQLError(e)) throw e;
      await wait(120);
    }
  }
  throw lastErr;
...
async function init() {
  ...
  initPromise = (async () => {
    ...
    const pool = getPool();
    const [rows] = await query(pool, 'SELECT COUNT(*) AS c FROM content_kv', [], 'mysql_content_init_timeout');
    ...
    const tasks = ALL_KEYS.map((k) => {
      const v = seed[k] || {};
      return query(pool, 'INSERT INTO content_kv (k, v) VALUES (?, ?)', [k, JSON.stringify(v)], 'mysql_content_seed_timeout');
    });
  ...
  initPromise.catch((e) => {
    initPromise = null;
    try {
      console.error('content_store_init_failed', e && e.stack ? e.stack : e);
    } catch (_err) {
      console.error('content_store_init_failed');
    }
  });
  return initPromise;
}
...
async function getSectionFromMySQL(key) {
  const pool = getPool();
  const [rows] = await query(pool, 'SELECT v FROM content_kv WHERE k = ?', [key], 'mysql_content_read_timeout');
...
async function setSectionToMySQL(key, value) {
  ...
  await query(pool, 'INSERT INTO content_kv (k, v) VALUES (?, ?) ON DUPLICATE KEY UPDATE v = VALUES(v)', [key, json], 'mysql_content_write_timeout');
  return true;
}
