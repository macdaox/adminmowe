...
- `COS_FILE_ENV_ID`（可选：COS 上传后返回 `cloud://` File ID 使用的云环境 ID；不填时会尝试从桶名推断）
...
- 管理员账号优先读取 MySQL `admin_users` 表，支持多个管理员账号和角色权限；首次没有数据库账号时读取 `ADMIN_EMAIL` / `ADMIN_PASSWORD`，在后台“基础设置 - 管理员账号”新增第一个账号后会写入 MySQL 并成为超级管理员，后续不再读取环境变量账号密码。普通管理员只能查看和修改自己的账号，超级管理员可以新增、编辑、删除其他管理员。
